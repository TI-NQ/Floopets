...
$directorio = "../View/img/imagen_raza/".$nombre_raza."/";
...
for($i=0; $i<$total; $i++) {
  ...
  $archivo     = $directorio.basename(str_replace(" ", "",$_FILES["ra_imagen"]["name"][$i]));
  $uploadOk    = 0;
  $extension_archivo = pathinfo($archivo,PATHINFO_EXTENSION);

  $check = getimagesize($_FILES["ra_imagen"]["tmp_name"][$i]);
      if($check !== false) {
...
// ASIGNAMOS UN LIMITE DE PESO A NUESTRO ARCHIVO ASIGNANDO UN VALOR EN Bytes
  if($_FILES["ra_imagen"]["size"][$i] > 2000000){
    echo "Ooops! tu imagen no puede superar mas de 200Kb  <br>";
    $uploadOk = 0;
  }
...
// VALIDAMOS SI $UPLOADOK ESTA EN 1 DE ESA FORMA SE PUEDE SUBIR
  ...
  if($uploadOk == 1){
      if (move_uploaded_file($_FILES["ra_imagen"]["tmp_name"][$i], $archivo)) {
          echo "El archivo ". basename( $_FILES["ra_imagen"]["name"][$i]). " se subio correctamente.  <br>";
      } else {
