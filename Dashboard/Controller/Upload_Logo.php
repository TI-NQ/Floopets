<?php
$total = count($_FILES['org_logo']['name']);

$directorio = "../View/img/imagen_organizacion/".$nombre_org_logo."/";
...
for($i=0; $i<$total; $i++) { 
  ...
  $archivo     = $directorio.basename(str_replace(" ", "",$_FILES["org_logo"]["name"][$i]));
  $uploadOk    = 0;
  $extension_archivo = pathinfo($archivo,PATHINFO_EXTENSION);
  
  $check = getimagesize($_FILES["org_logo"]["tmp_name"][$i]);
      if($check !== false) {
...
// ASIGNAMOS UN LIMITE DE PESO A NUESTRO ARCHIVO ASIGNANDO UN VALOR EN Bytes
  if($_FILES["org_logo"]["size"][$i] > 7000000){
...
// VALIDAMOS SI $UPLOADOK ESTA EN 1 DE ESA FORMA SE PUEDE SUBIR
  ...
  if($uploadOk == 1){
      if (move_uploaded_file($_FILES["org_logo"]["tmp_name"][$i], $archivo)) {
          echo "El archivo ". basename( $_FILES["org_logo"]["name"][$i]). " se subio correctamente.  <br>";
      } else {
