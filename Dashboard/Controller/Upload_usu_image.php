...
$directorio = "../View/img/imagen_usuario/".$nombre_usu_imagen."/";
...
for($i=0; $i<$total; $i++) { 
  ...
  $archivo     = $directorio.basename(str_replace(" ", "",$_FILES["usu_imagen"]["name"][$i]));
  $uploadOk    = 0;
  $extension_archivo = pathinfo($archivo,PATHINFO_EXTENSION);
  
  $check = getimagesize($_FILES["usu_imagen"]["tmp_name"][$i]);
      if($check !== false) {
...
// ASIGNAMOS UN LIMITE DE PESO A NUESTRO ARCHIVO ASIGNANDO UN VALOR EN Bytes
  if($_FILES["usu_imagen"]["size"][$i] > 7000000){
...
// VALIDAMOS SI $UPLOADOK ESTA EN 1 DE ESA FORMA SE PUEDE SUBIR
  ...
  if($uploadOk == 1){
      if (move_uploaded_file($_FILES["usu_imagen"]["tmp_name"][$i], $archivo)) {
          echo "El archivo ". basename( $_FILES["usu_imagen"]["name"][$i]). " se subio correctamente.  <br>";
      } else {
