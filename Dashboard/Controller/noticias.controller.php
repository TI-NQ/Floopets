<?php
	...
	switch ($accion) {
		...
		case 'u':
		$cod_noticias			=$_POST["cod_noticias"];
					$usu_cod_usuario = $_POST["usu_cod_usuario"];
			$not_titulo		=$_POST["not_titulo"];
			$not_contenido		=$_POST["not_contenido"];
			$not_galeria		=$_POST["not_galeria"];
			...
			$count_galeria			= count($_FILES['not_galeria']['name']);
				try{
					...
					Gestion_noticia::Update($cod_noticia,$usu_cod_usuario,$not_titulo,$not_contenido,$not_galeria,$not_portada,$not_palabras_clave);
		...
		case 'd':
					try {
		          $noticia = Gestion_noticia::Delete(base64_decode($_REQUEST["not"]));
